...
Oracle for HepG2 lentiMPRA activity: predicts a single scalar per 230bp one-hot input.
...
INPUT_LEN = 230
N_FEATURES = 1
DEFAULT_CONFIG = "/grid/koo/home/shared/d3/oracle_weights/lentimpra/config.json"
...
def load(checkpoint_path, device, config_path=None):
    cfg = _LegNetConfig.from_json(config_path or DEFAULT_CONFIG)
    model = cfg.get_model()
