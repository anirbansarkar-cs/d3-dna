...
def _pad_to(x, target_len):
    """Pad (N, 4, L) to (N, 4, target_len) with 0.25 background; truncate if too long."""
    L = x.shape[-1]
    if L >= target_len:
        return x[:, :, :target_len]
    pad_left = (target_len - L) // 2
    pad_right = target_len - L - pad_left
    left = np.full((x.shape[0], 4, pad_left), 0.25, dtype=np.float32)
    right = np.full((x.shape[0], 4, pad_right), 0.25, dtype=np.float32)
    return np.concatenate([left, x.astype(np.float32), right], axis=-1)


def _load_h3k4me3_mask(path):
    """Return a boolean mask of length 21907 selecting H3K4me3 tracks (2354 true).

    Parses target.sei.names: pipe-separated, column 1 is the mark/TF label.
    """
    flags = []
    with open(path) as f:
        for line in f:
            if not line.strip():
                continue
            parts = line.split("|")
            flags.append(len(parts) > 1 and parts[1].strip() == MASK_LABEL)
    mask = np.array(flags, dtype=bool)
    if mask.sum() != EXPECTED_MASK_SIZE:
        raise ValueError(
            f"Expected {EXPECTED_MASK_SIZE} H3K4me3 tracks in {path}, got {int(mask.sum())}"
        )
    return mask


class SeiOracle:
    """SEI oracle (DDSM protocol): predicts H3K4me3 mean activity scalar from (N,4,L) one-hot.

    Output shape (N, 1): mean across the 2350 H3K4me3 tracks.
    """

    def __init__(self, model, device, mask):
        self.model = model
        self.device = device
        self.mask = mask  # boolean array over 21907 features

    @torch.no_grad()
    def predict(self, x, batch_size=8, progress=True):
        x = np.asarray(x)
        x = _pad_to(x, INPUT_LEN)
        out = []
        it = range(0, len(x), batch_size)
        if progress:
            it = tqdm(it, desc="SEI predict")
        for i in it:
            b = torch.tensor(x[i : i + batch_size], dtype=torch.float32, device=self.device)
            scalar = self.model(b)[:, self.mask].mean(dim=1, keepdim=True)  # (B, 1)
            out.append(scalar.cpu().numpy())
