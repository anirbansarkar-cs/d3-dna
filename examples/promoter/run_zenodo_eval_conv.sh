#!/usr/bin/env bash
#SBATCH --job-name=promo_conv_zenodo
...
OUT_SAMPLES=generated_zenodo_conv
OUT_EVAL=eval_zenodo_conv
ORACLE_LOCAL=/grid/koo/home/shared/d3/oracle_weights/promoter/best.sei.model.pth.tar

echo "==[$(date)]== sample (convolutional, fp16/AMP, 100 steps, paired_repeat=1, --use-test-labels)"
python sample.py \
    --config config_conv.yaml \
    --use-test-labels \
    --steps 100 \
    --paired-repeat 1 \
